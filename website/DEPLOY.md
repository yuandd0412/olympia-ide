...
| 安装包(~49 MB,唯一版本) | Gitee Release(国内快)+ GitHub Releases 备份 | 超出 Pages 25 MiB 上限,不随官网发布 |
...
> 前置：发布产物必须先用 `scripts/strip-toolchain.py` 裁剪工具链后再
> `npm run tauri build`，否则 ~105 MB 超 Gitee 单文件上限（裁剪后实测 49 MB）。
...
2. `npm run tauri build`（唯一版本，内置工具链，发布前确认已跑 `python scripts/strip-toolchain.py`）；
3. 更新 `index.html` 中的版本号与下载文件名，新安装包以新文件名上传 Gitee / GitHub Releases（保留旧版本供回滚）；
4. `npx wrangler pages deploy . --project-name olympia-ide --branch main` 重新发布官网。

## 备注

- 自动下载流程（检测不到工具链时的引导）使用 MinGW 13.1.0（sha256
  `6b41fdf2…`），走清华 TUNA 镜像；如需自托管可上传同一 7z 到 R2
